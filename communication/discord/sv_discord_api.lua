local token = "Bot " .. ConfigDiscord.DiscordToken
...
function getRoles( user )
    ...
    if identifiers.discord then
        local endpoint = ( "guilds/%s/members/%s" ):format( ConfigDiscord.GuildID, identifiers.discord )
...
function isRolePresent( user, role )
    ...
    if identifiers.discord then
        ...
        if result.code == 200 then
            local data = json.decode( result.data )
            local toFind = ConfigDiscord.DiscordRoles[ tostring( role ) ]
...
Citizen.CreateThread( function()
    local guild = DiscordRequest( "guilds/" .. ConfigDiscord.GuildID, "GET", {} )
